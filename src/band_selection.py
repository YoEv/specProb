...
if __name__ == "__main__":
    ...
    for i, band in enumerate(tqdm(initial_bands, desc="Evaluating Bands")):
        ...
        print(f"Band {i} {band}: Accuracy = {accuracy:.4f}")
    ...
    for item in sorted_bands:
        print(f"Band {item['band_index']} {item['band_range']}: Accuracy = {item['accuracy']:.4f}")
    ...
    for i, band_idx in enumerate(tqdm(sorted_band_indices, desc="Cumulative Probing")):
        ...
        print(f"Accumulating Band {band_idx} (Top {i+1}). Total Features: {cumulative_features.shape[1]}. Accuracy: {accuracy:.4f}")
