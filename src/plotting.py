...
def create_and_save_plot(target_genre, bar_labels, bar_heights, colors, normalized_weights, chance, n_coeffs, output_dir):
    ...
    for j in range(len(normalized_weights) - 1):
        avg_weight = (normalized_weights[j] + normalized_weights[j+1]) / 2
        ax2.fill_between([j, j+1], [normalized_weights[j], normalized_weights[j+1]], 0, color=cmap(norm(avg_weight)), alpha=0.7)
